...
router.get("/tasks", async (req, res): Promise<void> => {
  ...
  let tasks = await db.select().from(tasksTable).orderBy(tasksTable.dueDate);

  if (qp.success) {
    if (qp.data.matterId) tasks = tasks.filter((t) => t.matterId === qp.data.matterId);
    if (qp.data.assigneeId) tasks = tasks.filter((t) => t.assigneeId === qp.data.assigneeId);
    if (qp.data.status) tasks = tasks.filter((t) => t.status === qp.data.status);
    if (qp.data.myTasks === "true") tasks = tasks.filter((t) => t.assigneeId === req.user!.id);
  }
